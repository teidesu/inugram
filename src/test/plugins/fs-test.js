...
function main() {
  ...
  inu.fs.append('blob.bin', new Blob(['!']))
  check('a Blob writes and appends as its content', text.decode(inu.fs.read('blob.bin')) === 'abcdef!', text.decode(inu.fs.read('blob.bin')))
  ...
  const listed = JSON.stringify(inu.fs.readdir('a/b/c'))
  check('readdir gives names, sorted, without . or ..', listed === '["d","one.txt","two.txt"]', listed)
  ...
  check('a write past the cap is refused', quotaError instanceof inu.PluginError && quotaError.code === 'quota-exceeded', `${quotaError}`)
  check('having written nothing', !inu.fs.exists('huge.bin') && inu.fs.usage() === before, inu.fs.usage())
  check('and it names the numbers', quotaError.usage === before + quota + 1 && quotaError.quota === quota, `${quotaError.usage}/${quotaError.quota}`)
  ...
  // absolute paths outside the plugin root: refused on the string, `fs` buys none of them
  for (const call of [() => inu.android.getPluginsDir(), () => inu.android.getCacheDir(), () => inu.android.getMediaDir('images')]) {
