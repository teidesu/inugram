...
// the fake host answers `/refuse` with the address refusal, never answers `/never`, and
// everything else with the same json 200
...
async function main() {
  ...
  check('getSetCookie() keeps each cookie apart', res.headers.getSetCookie().join('|') === 'a=1|b=2', JSON.stringify(res.headers.getSetCookie()))
  let immutable = false
  try {
  ...
  await expectReject('an address the host refuses reaches the plugin as forbidden', 'forbidden', fetch('https://example.com/refuse'))
