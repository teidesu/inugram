...
mod wiring {
  ...
  fn installing_the_api_installs_the_sandbox_globals_and_the_timer_wheel() {
    ...
    assert!(body.contains("install_globals("), "the documented sandbox globals would be missing");
    assert!(body.contains("install_timers("), "setTimeout would be missing");
    assert!(
      body.contains("install_utils_with_host(&ctx, utils_host, &inu)")
        && body.contains("install_message(&ctx, &shared, &inu)"),
    ...
    assert!(
      body.contains("PathBuf::from(spill_dir)")
        && body.contains("install_globals(&ctx, random_host, &spill_dir, external.clone())"),
  ...
  fn installing_the_api_installs_the_account_read_surface() {
    let body = body_of("nativeInstallApi");
    let accounts = position_of(body, "install_account(", "inu.account would be missing");
    let reads = position_of(body, "install_reads(", "the Account getters would be missing");
  ...
  fn a_visibility_change_reaches_the_wheel_before_the_plugin() {
    ...
    let callbacks =
      position_of(body, "crate::api::lifecycle::app_visibility_changed(", "inu.onAppVisibilityChange never fires");
  ...
  fn every_kotlin_native_has_a_rust_export() {
    let exports: std::collections::HashSet<_> = rust_exports().iter().map(|s| export_package_and_method(s)).collect();
