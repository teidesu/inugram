...
fn setup(grants: &[&str]) -> Fixture {
    ...
    let state = ctx.with(|ctx| {
        ...
        install_deserialize(&ctx, host_dyn, grants, lifecycle.clone(), views, log).unwrap()
    });
    Fixture { _rt: rt, ctx, host, lifecycle, state, tl, logs }
}
...
fn a_rule_crosses_as_normalized_json() {
    let f = setup(&[GRANT]);
    eval(&f, "inu.interceptDeserialize([{ type: 'user', when: { self: true }, set: { premium: true } }]), 'ok'");
    assert_eq!(f.host.registered.borrow()[0].1, r#"[{"type":["user"],"when":{"self":true},"set":{"premium":true}}]"#,);
}
...
fn a_disposed_middleware_is_never_dispatched_again() {
    ...
    f.ctx.with(|ctx| {
        ctx.eval::<(), _>(
            "globalThis.__runs = 0; globalThis.__d = inu.interceptDeserialize(['user'], () => { __runs++ })",
        )
        .unwrap();
    });
    dispatch_middleware(&f.ctx, &f.state, 1, "HOW7");
...
fn a_middleware_counts_against_the_same_rule_budget() {
    ...
    assert_eq!(catch(&f, "inu.interceptDeserialize([{ type: 'user', set: { premium: true } }])").0, "quota-exceeded",);
}
...
fn the_shape_refusals_all_name_the_rule_that_broke() {
    ...
    for (code, expected) in [
        ...
        ("inu.interceptDeserialize([{ type: [], set: { premium: true } }])", "rules[0].type names no constructor"),
        (
            "inu.interceptDeserialize([{ type: [1], set: { premium: true } }])",
            "rules[0].type must contain only constructor names",
        ),
        ("inu.interceptDeserialize([{ type: 'user' }])", "rules[0].set must be an object of field names to constants"),
...
fn the_ceiling_is_the_one_the_contract_states() {
    assert_eq!(
        crate::testing::util::stated_number(crate::testing::util::CONTRACT, "at most {} rules live at once"),
        MAX_RULES as u64,
    );
}
...
mod bundled_oracle {
    ...
    fn the_bundled_deserialize_test_plugin_passes() {
        ...
        ctx.with(|ctx| {
            install_plugin_error(&ctx).unwrap();
            let tl = TlViews::new(Rc::new(super::tests::TestTl::default()) as Rc<dyn crate::tl::proxy::TlHost>);
            let log: std::sync::Arc<dyn Fn(&str) + Send + Sync> = std::sync::Arc::new(|_: &str| {});
            install_deserialize(&ctx, host_dyn, grants, Lifecycle::new(), tl, log).unwrap();
        });

        let lines = crate::testing::util::run_capturing_console(&rt, &ctx, ORACLE);
        crate::testing::util::assert_oracle_exact(&lines, "deserialize test done", 21);
