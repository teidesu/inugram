...
use crate::tg::writes::json_string;
...
fn read_types<'js>(ctx: &Ctx<'js>, at: &str, value: Value<'js>) -> JsResult<Vec<String>> {
    ...
    let mut out = Vec::new();
    for item in crate::engine::argv::array_values(ctx, array, &format!("{at}.type"))? {
...
fn read_rules<'js>(ctx: &Ctx<'js>, value: Value<'js>) -> JsResult<Vec<Rule>> {
    ...
    let rules = crate::engine::argv::array_values(ctx, array, "interceptDeserialize")?;
    if rules.is_empty() {
...
fn register_middleware<'js>(
    ...
    for item in crate::engine::argv::array_values(ctx, array, "interceptDeserialize")? {
        let Some(name) = item.as_string() else {
