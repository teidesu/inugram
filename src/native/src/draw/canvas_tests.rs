...
impl<'a> Reader<'a> {
    ...
    fn paint(&mut self) -> Paint {
        ...
        Paint { alpha, composite, shadow, shadow_color, kind, color, coords, stops, pattern }
    }
    ...
    fn stroke(&mut self) -> Stroke {
        ...
        Stroke { width, cap, join, miter, dash_offset, dash }
    }
    ...
    while reader.at < bytes.len() {
        ...
        out.push(match command {
            ...
            CMD_STROKE => {
                ...
                Command::Stroke { matrix, paint, stroke, path: reader.path() }
            }
            CMD_CLIP => {
                let matrix = reader.matrix();
                let rule = reader.u8();
...
fn setup(name: &str) -> Fixture {
    ...
    let state = ctx.with(|ctx| {
        ...
        let blobs = crate::io::blob::install(&ctx, dir.path(), external.clone()).unwrap();
...
fn the_gradient_stop_ceiling_is_the_one_the_contract_states() {
    let stated =
        crate::testing::util::stated_number(include_str!("../../../plugins/canvas.d.ts"), "at most {} colour stops");
...
fn a_path_built_across_two_transforms_keeps_each_point_where_it_was_put() {
    ...
    let commands =
        draw(&f, "x.beginPath(); x.moveTo(0, 0); x.translate(50, 0); x.lineTo(0, 0); x.resetTransform(); x.stroke()");
...
fn text_crosses_as_an_interned_string_with_its_font_and_its_placement() {
    ...
    let Command::Text { font, text, align, baseline, x, y, max_width, stroked, .. } = &commands[0] else {
        panic!("{commands:?}")
    };
...
mod bundled_oracle {
    ...
    fn the_bundled_canvas_test_plugin_passes() {
        ...
        crate::testing::util::assert_oracle_exact(&lines, "canvas test done", 65);
    }
