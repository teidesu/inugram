...
fn the_default_quota_is_the_size_the_contract_states() {
    use crate::testing::util::{stated_number, FS_CONTRACT};
...
fn unscoped_mode_reaches_outside_and_relative_paths_still_land_inside() {
    ...
    let secret = f.outside_path.join("secret.txt");
    assert_eq!(eval(&f, &format!("new TextDecoder().decode(inu.fs.read({:?}))", secret.to_string_lossy())), "a secret",);
...
mod bundled_oracle {
    ...
    fn oracle_quota() -> u64 {
        ...
        for token in crate::testing::util::manifest_grants(ORACLE) {
    ...
    fn the_bundled_fs_test_plugin_passes() {
        ...
        crate::testing::util::assert_oracle_exact(&lines, "fs test done", 60);
    }
}
