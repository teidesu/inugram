...
fn a_spill_past_the_ceiling_is_refused_and_leaves_nothing_behind() {
    let f = setup_with(
        "spill-limit",
        true,
        BlobLimits { spill_bytes: SPILL_THRESHOLD_BYTES + 1024, ..BlobLimits::default() },
    );
    let out = eval(
        &f,
        &format!(
...
fn with_nowhere_to_spill_the_buffer_is_charged_as_it_grows() {
    ...
    assert!(consumed < 16, "the construction has to stop at the part it cannot afford, not read all of them: {out}",);
...
fn a_construction_past_what_one_call_may_move_is_refused_before_it_reads() {
    let f = setup_with("build-limit", true, BlobLimits { build: 64 * 1024, ..BlobLimits::default() });
...
fn an_unreachable_spill_is_collected_before_either_ceiling_refuses() {
    ...
    let f = setup_with(
        "spill-bytes-gc",
        true,
        BlobLimits { spill_bytes: 2 * SPILL_THRESHOLD_BYTES + 1, ..BlobLimits::default() },
    );
    run(&f, &orphan);
    assert_eq!(f.state.spilled_bytes(), SPILL_THRESHOLD_BYTES + 1);
...
fn disposing_a_root_kills_every_slice_s_reads_but_not_their_metadata() {
    ...
    assert_eq!(settle(&f, "__s.bytes()"), "PluginError:handle-expired:the blob this content belongs to was disposed",);
...
fn cloning_gives_an_independent_handle_over_the_same_content() {
    ...
    assert_eq!(eval(&f, "[__c !== __b, __c.size, __c.type, __c instanceof Blob].join('|')"), "true|5|text/plain|true",);
...
fn an_exported_id_carries_the_absolute_range_and_is_per_engine() {
    ...
    assert!(resolve_export(&other.state, 2).is_none(), "an id is meaningless to any engine but the one that minted it",);
...
mod bundled_oracle {
    ...
    fn the_bundled_blob_test_plugin_passes() {
        ...
        ctx.with(|ctx| {
            ...
            crate::io::blob::install(&ctx, dir.path(), ExternalMemory::new()).unwrap();
            let clone = make_clone_fn(&ctx).unwrap();
        ...
        crate::testing::util::assert_oracle_exact(&lines, "blob test done", 52);
    }
}
...
mod label_tests {
    ...
    fn a_label_cannot_carry_unbounded_content_into_the_handle() {
        let stated =
            crate::testing::util::stated_number(crate::testing::util::CONTRACT, "**truncated at {} characters**")
                as usize;
