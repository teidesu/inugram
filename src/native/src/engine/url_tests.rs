...
mod parsing {
    ...
    /// `crate::engine::argv`'s hazard on this surface: webidl reads an explicit `undefined` as absent
    #[test]
...
mod search_params {
    ...
    fn it_iterates_every_way_the_spec_offers() {
        ...
        assert_eq!(eval(&ctx, &format!("{p} const o = []; p.forEach((v,k) => o.push(k+'='+v)); o.join('&')")), "a=1&b=2");
    }
