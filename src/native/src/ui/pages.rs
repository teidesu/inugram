...
fn check_slider_steps<'js>(ctx: &Ctx<'js>, min: f64, max: f64, step: f64) -> JsResult<()> {
    ...
    if steps > MAX_SLIDER_LABELS {
        return throw_plugin_error(
            ...
            "invalid-argument",
            &format!(
                "slider: a 'label' is evaluated per step, and {steps} steps is past the {MAX_SLIDER_LABELS} allowed"
            ),
            None,
            None,
            None,
...
pub fn install_ui<'js>(
    ...
    }
    {
        ...
        inu.set(
            "registerSettings",
            Function::new(ctx.clone(), move |ctx: Ctx<'js>, page: Value<'js>| {
                js_register_settings(&ctx, &state2, page)
            })?,
        )?;
    }
...
fn java_handle<'js>(ctx: &Ctx<'js>, state: &Rc<UiState>, value: &Value<'js>) -> JsResult<Option<i64>> {
    ...
    let id = crate::platform::jvm::handle_id(ctx, jvm, value)?;
...
fn js_open_menu<'js>(ctx: &Ctx<'js>, state: &Rc<UiState>, page_id: i64, row: &str, items: Value<'js>) -> JsResult<()> {
    ...
    state.menus.borrow_mut().insert(menu_id, callbacks.into_iter().map(|f| Persistent::save(ctx, f)).collect());
    Ok(())
}
