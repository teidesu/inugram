...
type Disposing = crate::testing::util::DisposeOnDrop<UiState>;

type Fixture = (Runtime, Context, Rc<TestUiHost>, Disposing, std::sync::Arc<crate::testing::util::Logs>);
...
fn a_java_object_reaches_open_page_and_native_view() {
    ...
    let host = Rc::new(TestUiHost::default());
    let host_dyn: Rc<dyn UiHost> = host.clone();
    let log = crate::testing::util::log_sink(&crate::testing::util::Logs::new());
    let jvm_host = crate::platform::jvm::tests::testing::OracleJvmHost::new();
    let grants = crate::engine::error::TestGrantHost::new(&["unsafe.jvm"]);
    ...
    let state = Disposing::new(&ctx, state, dispose);
    let _jvm = crate::testing::util::DisposeOnDrop::new(&ctx, jvm, crate::platform::jvm::dispose);
...
fn the_slider_label_cap_is_the_one_the_contract_states() {
    let stated =
        crate::testing::util::stated_number(crate::testing::util::CONTRACT, "may have at most {} steps**") as usize;
...
fn the_bundled_ui_test_plugin_passes() {
    ...
    let lines = crate::testing::util::run_capturing_console(&rt, &ctx, &source);

    crate::testing::util::assert_oracle_exact(&lines, "ui test done", 17);
