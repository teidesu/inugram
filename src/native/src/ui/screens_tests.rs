...
type Disposing = crate::testing::util::DisposeOnDrop<ScreenState>;
type AccountDisposing = crate::testing::util::DisposeOnDrop<AccountState>;
type Fixture =
    (Runtime, Context, Rc<TestScreenHost>, Disposing, AccountDisposing, std::sync::Arc<crate::testing::util::Logs>);
...
fn the_bundled_nav_test_plugin_passes() {
    let (rt, ctx, host, state, _accounts, logs) = setup(&crate::testing::util::manifest_grants(ORACLE));
    *host.screen.borrow_mut() = format!("J{DIALOGS}");
    ...
    crate::testing::util::assert_oracle_exact(&lines, "nav test done", 38);
    assert!(logs.borrow().is_empty(), "unexpected logs: {:?}", logs.borrow());
}
