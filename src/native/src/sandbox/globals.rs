...
//! (those are [`crate::sandbox::timers`]).
//!
...
//! `URL`/`URLSearchParams` ([`crate::sandbox::url`]), which needs a real parser rather than a shape.
...
///
/// The blob state is returned rather than dropped because [`crate::telegram::writes`] needs it: a `Blob` in a
...
pub fn install_globals<'js>(
...
) -> JsResult<Rc<crate::io::blob::BlobState>> {
    ...
    crate::sandbox::url::install_url(ctx)?;
