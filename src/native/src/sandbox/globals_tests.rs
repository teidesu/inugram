...
impl Default for TestRandomHost {
    fn default() -> Self {
        TestRandomHost { next: RefCell::new(1), available: RefCell::new(true) }
    }
}
...
fn installing_into_a_context_without_the_intrinsics_refuses() {
    ...
    let message = ctx.with(|ctx| {
        ...
        crate::telegram::rpc::format_exception(&ctx)
    });
...
impl crate::tl::proxy::TlHost for OneMessageTl {
...
fn structured_clone_throws_on_a_tl_view() {
    use crate::tl::proxy::{wire_to_js_value, TlHost, TlViews, ViewLife};
...
fn an_app_owned_file_outlives_every_blob_over_it() {
    ...
    std::fs::write(&path, b"0123456789").unwrap();
    {
        let (rt, ctx, _host) = setup();
        ctx.with(|ctx| {
            let value = crate::io::blob::mint_app_file(
                &ctx,
                &path,
                10,
                "image/jpeg",
...
fn a_build_that_throws_after_it_started_spilling_leaves_nothing_behind() {
    ...
    let out = eval(
        ...
        &format!(
            ...
            out;
            "#,
            crate::io::blob::SPILL_THRESHOLD_BYTES + 1,
        ),
    );
    ...
    run(&ctx, &format!("globalThis.__b = new Blob([new Uint8Array({})]);", crate::io::blob::SPILL_THRESHOLD_BYTES + 1));
...
fn with_nowhere_to_spill_a_full_native_budget_is_the_one_way_building_a_blob_fails() {
    ...
    run(
        ...
        &format!(
            ...
            "#,
            crate::io::blob::SPILL_THRESHOLD_BYTES,
            crate::sandbox::limits::EXTERNAL_LIMIT_BYTES as u64 / crate::io::blob::SPILL_THRESHOLD_BYTES,
        ),
    );
    let out = eval(
        &ctx,
