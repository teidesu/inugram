...
mod parsing {
    ...
    /// `crate::sandbox::argv`'s hazard on this surface: webidl reads an explicit `undefined` as absent
    #[test]
...
mod setters {
    ...
    fn the_host_setter_takes_a_port_and_the_hostname_setter_does_not() {
        ...
        assert_eq!(
            eval(&ctx, "const u = new URL('https://a.com/'); u.hostname = 'b.com:81'; u.href"),
            "https://b.com/"
        );
...
mod search_params {
    ...
    fn the_params_object_is_the_same_one_every_time() {
        let (_rt, ctx) = setup();
        assert_eq!(
            eval(&ctx, "const u = new URL('https://a.com/'); String(u.searchParams === u.searchParams)"),
            "true"
        );
        let src = r"
