...
//! construct, `E`/`R`/`P` the three error shapes decoded by [`crate::sandbox::error`]) rather than raw JSON -
...
use crate::telegram::account::{dispatch_account, AccountState};
...
fn install_account_invoke<'js>(ctx: &Ctx<'js>, state: &Rc<RpcState>) -> JsResult<()> {
    ...
    if let Some(inner) = crate::telegram::account::take_prototype(ctx, &accounts) {
...
fn install_send_message<'js>(
    ...
    let factory = crate::sandbox::prelude::load(ctx, SEND_PRELUDE)?;
    ...
    let self_user_id = Function::new(ctx.clone(), move |account_id: i32| {
        crate::telegram::account::self_user_id(&accounts, account_id).map(|id| id as f64)
    })?;
...
fn install_demuxed_events<'js>(ctx: &Ctx<'js>, state: &Rc<RpcState>, inu: &Object<'js>) -> JsResult<()> {
    let factory = crate::sandbox::prelude::load(ctx, EVENTS_PRELUDE)?;
...
fn read_name_list<'js>(ctx: &Ctx<'js>, what: &str, names: Value<'js>, noun: &str) -> JsResult<Vec<String>> {
    let list: Vec<String> = if let Some(s) = names.as_string() {
        ...
        let mut out = Vec::new();
        for item in crate::sandbox::argv::array_values(ctx, arr, what)? {
            let s = item
                .as_string()
...
pub fn dispose(context: &rquickjs::Context, state: &Rc<RpcState>) {
    context.with(|ctx| {
        ...
        if let Some(accounts) = state.accounts.as_ref() {
            let _ = crate::telegram::account::take_prototype(&ctx, accounts);
        }
