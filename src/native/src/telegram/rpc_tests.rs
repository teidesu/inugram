...
fn setup_logging(grants: &[&str]) -> LoggingFixture {
    ...
    let accounts_host: Rc<dyn crate::telegram::account::AccountHost> =
        crate::telegram::account::tests::TestAccountHost::with(crate::telegram::account::tests::TWO_ACCOUNTS);
...
fn the_bundled_accounts_test_plugin_passes() {
    ...
    let accounts_host =
        crate::telegram::account::tests::TestAccountHost::with(crate::telegram::account::tests::TWO_ACCOUNTS);
    let accounts_dyn: Rc<dyn crate::telegram::account::AccountHost> = accounts_host.clone();
    ...
    let _accounts_disposer =
        crate::testing::util::DisposeOnDrop::new(&ctx, accounts.clone(), crate::telegram::account::dispose);
    let state = Disposing::new(&ctx, state, dispose);
    ...
    *accounts_host.json.borrow_mut() = SWITCHED.to_string();
    crate::telegram::account::accounts_changed(&rt, &ctx, &accounts);
    dispatch_update(
        &rt,
        &ctx,
        &state,
    ...
    fn setup(grants: &[&str], with_globals: bool) -> Fixture {
        ...
        let state = ctx.with(|ctx| {
            ...
            if with_globals {
                ...
                crate::sandbox::globals::install_globals(
                    &ctx,
                    random,
                    std::path::Path::new(""),
