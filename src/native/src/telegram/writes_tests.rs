...
impl ReadsHost for SelfOnlyReadsHost {
    fn account_read(&self, _account_id: i32, op: i32, arg: &str) -> String {
        ...
        if op != crate::telegram::reads::OP_INPUT_PEER {
            return "N".to_string();
        }
        if spec == "S" {
...
type Fixture = (
    ...
    crate::testing::util::DisposeOnDrop<crate::telegram::reads::ReadsState>,
    crate::testing::util::DisposeOnDrop<crate::telegram::account::AccountState>,
    tempdir::TempDir,
);
...
fn setup_with_limit(grants: &[&str], transfer_limit: u64) -> Fixture {
    ...
    let (writes, reads, accounts) = ctx.with(|ctx| {
        ...
        let blobs = crate::sandbox::globals::install_globals(
            &ctx,
            random,
            dir.path(),
    ...
    let reads = crate::testing::util::DisposeOnDrop::new(&ctx, reads, crate::telegram::reads::dispose);
    let accounts = crate::testing::util::DisposeOnDrop::new(&ctx, accounts, crate::telegram::account::dispose);
...
impl crate::telegram::rpc::RpcHost for TestRpcHost {
...
type SendFixture = (
    ...
    Context,
    Rc<TestRpcHost>,
    crate::testing::util::DisposeOnDrop<crate::telegram::rpc::RpcState>,
    crate::testing::util::DisposeOnDrop<crate::telegram::reads::ReadsState>,
    crate::testing::util::DisposeOnDrop<crate::telegram::account::AccountState>,
);
...
fn setup_send(grants: &[&str]) -> SendFixture {
    ...
    let (rpc, reads, accounts) = ctx.with(|ctx| {
        ...
        .unwrap();
        let rpc_host_dyn: Rc<dyn crate::telegram::rpc::RpcHost> = rpc_host.clone();
        let rpc = crate::telegram::rpc::install_rpc(
            &ctx,
            rpc_host_dyn,
            views.clone(),
            grants,
    ...
    let rpc = crate::testing::util::DisposeOnDrop::new(&ctx, rpc, crate::telegram::rpc::dispose);
    let reads = crate::testing::util::DisposeOnDrop::new(&ctx, reads, crate::telegram::reads::dispose);
    let accounts = crate::testing::util::DisposeOnDrop::new(&ctx, accounts, crate::telegram::account::dispose);
...
fn run_one_send(fixture: &SendFixture, middleware: &str) -> Option<String> {
    ...
    crate::telegram::rpc::dispatch_rpc(rt, ctx, rpc, callback_id, 1, "messages.sendMessage", 0, &format!("J{A_SEND}"));
