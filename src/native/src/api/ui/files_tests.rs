...
fn a_picked_file_belongs_to_the_plugin_and_the_copy_goes_when_the_file_does() {
  ...
  let request = pick(&f, "{}");
  answer(&f, request, &format!(r#"[{{"path":"{}","name":"owned.bin","type":""}}]"#, one.to_string_lossy()), None);
...
fn an_answer_this_cannot_read_rejects_rather_than_leaving_the_promise_hanging() {
  ...
  answer(&f, request, &format!(r#"[{{"path":"{}","name":"gone.bin","type":""}}]"#, gone.to_string_lossy()), None);
