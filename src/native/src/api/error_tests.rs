...
fn plugin_error_wire_empty_fields_become_absent_props() {
    ...
    let has_own = ctx.with(|ctx| {
        let value =
            wire_error_to_js(&ctx, "Pnot-granted\nkv\n\n\nmissing grant: kv").expect("expected an error wire").unwrap();
        ctx.globals().set("e", value).unwrap();
...
fn the_host_error_channel_still_decodes_a_plugin_error_wire() {
    ...
    let got = ctx.with(|ctx| {
        ...
        ctx.globals().set("e", value).unwrap();
        ctx.eval::<String, _>("[e instanceof inu.PluginError, e.code, e.grant, e.usage, e.quota, e.message].join('|')")
            .unwrap()
    });
