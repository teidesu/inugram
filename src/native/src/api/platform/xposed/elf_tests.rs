...
fn a_zero_valued_symbol_is_not_an_answer() {
    let image =
        Builder::new().table(".symtab", &[("_ZN3art3Foo3BarEv", 0, true), ("_ZN3art3Foo3BarEv", 0x40, true)]).build();
    let symbols = Symbols::parse(image).expect("parses");
...
fn the_bias_is_added_to_every_answer() {
    ...
    let loaded = LoadedImage { bias: 0x7000, symbols: Symbols::parse(image).expect("parses") };
...
fn exact_and_prefix_answers_do_not_share_a_cache_entry() {
    ...
    let loaded = LoadedImage { bias: 0, symbols: Symbols::parse(image).expect("parses") };
    let mut resolver = Resolver { image: Some(loaded), seen: HashMap::new() };
