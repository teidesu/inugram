...
//!
//! A response body is a file the host wrote, reaching js as a [`crate::api::io::blob`] app-file backing, so
//! it never touches either heap. A *request* body does cross, bounded by
//! [`crate::api::io::blob::BUILD_LIMIT_BYTES`]; anything bigger is `uploadFile`'s job.
...
/// The host the grant is checked against, from [`crate::api::url`]'s screen - which `openUrl`
...
fn js_send<'js>(
    ...
    // `Ctx::json_stringify` rather than `globalThis.JSON`, which is writable and shared with the
    // plugin: the prelude runs in the plugin's realm, so a spec it serialized itself would be
    // whatever the plugin's `JSON.stringify` felt like returning. The host validates it either way.
