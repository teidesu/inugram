...
impl OracleXposedHost {
    fn new() -> Rc<Self> {
        Rc::new(OracleXposedHost { sites: RefCell::new(HashMap::new()), next: RefCell::new(100) })
    }
...
fn the_bundled_xposed_test_plugin_passes() {
    ...
    let grants = crate::grants::TestGrantHost::new(&manifest_grants(ORACLE)).as_host();
    ...
    let _jvm = DisposeOnDrop::new(&ctx, jvm, crate::platform::jvm::dispose);
...
fn run_dispatch(
    ...
    let result = if answer[0] == "P1" { dispatch_after(rt, context, state, 1, original) } else { original.to_string() };
...
fn setup(grants: &[&str]) -> Fixture {
    let rt = Runtime::new().unwrap();
    let ctx = Context::full(&rt).unwrap();
    let grant_host = crate::grants::TestGrantHost::new(grants).as_host();
    let lifecycle = Lifecycle::new();
...
fn a_throwing_hook_is_a_fault_and_the_original_still_runs() {
    ...
    assert!(
        logs.iter().any(|line| crate::classify_log(line).0 == crate::LEVEL_FAULT && line.contains("before hook threw")),
        "{logs:?}"
    );
}
