...
class PluginRequestFlagsTest {
    ...
    @Test
    fun `a send carrying a reply, entities and a schedule sets their bits`() {
        val plugin = granted()
    ...
    @Test
    fun `a topic reply sets the bit on the nested reply, not only the request`() {
        val plugin = granted()
    ...
    @Test
    fun `a reaction carries its reaction vector`() {
        val plugin = granted()
    ...
    @Test
    fun `every request the write surface sends agrees with its own fields`() {
        val plugin = granted()
        val entity = JSONObject().put("_", "messageEntityBold").put("offset", 0).put("length", 2)
        val reaction = JSONObject().put("_", "reactionEmoji").put("emoticon", "👍")
