...
class PluginReadsTest {
    ...
    fun setUp() {
        ...
        controller.inu_putChat(basicGroup(2002L))
    }
    ...
    @Test
    fun `no read reaches an encrypted dialog, whichever op names it`() {
    ...
    @Test
    fun `an ordinary dialog id is not mistaken for an encrypted one`() {
        val plugin = granted()
    ...
    @Test
    fun `every op checks its own scope on the side that owns the data`() {
     ...
     */
    @Test
    fun `naming yourself takes the self scope on top of the reads own`() {
        ...
        MediaDataController.getInstance(0).inu_putDraft(self, 0L, TLRPC.TL_draftMessage().apply {
            message = "unsent"
        }.synced())
    ...
    @Test
    fun `a peer spec is resolved the three ways the prelude can write one`() {
        val plugin = granted()
    ...
    @Test
    fun `a miss is null rather than an error`() {
        val plugin = granted()
     ...
     */
    @Test
    fun `a zero dialog id is a miss, not the logged-in user`() {
        val plugin = startPlugin("peers", "account.read(peers)")
    ...
    @Test
    fun `the entity getters do not cross kinds`() {
        val plugin = granted()
    ...
    @Test
    fun `a batch answers one wire per element and keeps its misses in place`() {
        val plugin = granted()
    ...
    @Test
    fun `a chat batch answers for chats alone`() {
        val plugin = granted()
    ...
    private fun cacheMessage(dialogId: Long, message: TLRPC.Message) {
        val controller = MessagesController.getInstance(0)
        controller.dialogMessage.put(dialogId, arrayListOf(MessageObject(message)))
    }

    @Test
    fun `a dialog and its cached message read back`() {
        val plugin = granted()
        cacheDialog(self, 7)
        cacheMessage(self, TLRPC.TL_message().apply { id = 7; message = "hi" }.synced())
    ...
    @Test
    fun `a message batch keeps its misses in place too`() {
        val plugin = granted()
        cacheMessage(self, TLRPC.TL_message().apply { id = 7; message = "hi" }.synced())
     ...
     */
    @Test
    fun `a dialog row carries no draft without the draft scope`() {
        val plugin = granted()
    ...
    @Test
    fun `everything read off an account refuses writes`() {
        val plugin = granted()
    ...
    @Test
    fun `a login code is redacted out of a message read through a getter`() {
        val plugin = granted()
    ...
    @Test
    fun `an uncached peer has no input peer, however willing stock is to build one`() {
        val plugin = granted()
    ...
    @Test
    fun `narrowing to the wrong kind says so instead of answering nothing`() {
        val plugin = granted()
    ...
    @Test
    fun `only a username can be looked up`() {
        val plugin = granted()
    ...
    @Test
    fun `resolving a username sends one request and settles from its answer`() {
        val plugin = granted()
    ...
    @Test
    fun `a username that resolves to the wrong kind is an invalid argument`() {
        val plugin = granted()
