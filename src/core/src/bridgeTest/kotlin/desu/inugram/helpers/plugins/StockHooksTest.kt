 ...
 * Every other file here drives the bridge directly, and the harness's own `ConnectionsManager`
 * calls the interception hook itself, so a stock call site that goes away during a rebase takes
 * the whole api with it while every suite stays green - which is exactly how `maybeIntercept` was
 * once absent from the worktree with nothing to show for it.
 *
...
class StockHooksTest {
    private val stockRoot = File(forkRoot(), "worktree/TMessagesProj/src/main/java")

    private fun stock(path: String): String = File(stockRoot, path).readText()
    ...
    private fun blockAt(source: String, open: Int): IntRange {
        var i = open + 1
        var depth = 1
        while (i < source.length) {
            when (source[i]) {
    ...
    private fun assertOpensWith(body: String, statement: String, why: String) =
    ...
    fun `stock reaches the plugin engine from exactly these places`() {
        ...
        stockRoot.walkTopDown().filter { it.isFile && it.extension == "java" }.forEach { file ->
            val source = file.readText()
            if (!source.contains("desu.inugram.")) return@forEach
            val path = file.relativeTo(stockRoot).invariantSeparatorsPath
