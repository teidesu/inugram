...
class PluginFetchTest {
     ...
     */
    @Test
    fun `a redirect off the granted domain fails the request and is never sent`() {
    ...
    @Test
    fun `every hop of a chain that stays in scope is screened and followed`() {
     ...
     */
    @Test
    fun `a hop whose host resolves into a private range is refused mid-chain`() {
    ...
    @Test
    fun `manual hands the redirect back and error refuses it`() {
    ...
    @Test
    fun `a chain that never ends is stopped after the number of hops the contract states`() {
     ...
     */
    @Test
    fun `a see-other turns the request into a bodyless GET`() {
    ...
    @Test
    fun `a temporary redirect keeps the method and the body`() {
     ...
     */
    @Test
    fun `a redirect off http entirely is refused and never followed`() {
     ...
     */
    @Test
    fun `an abort before the first hop stops the request without sending it`() {
     ...
     */
    @Test
    fun `an abort that lands while the name resolves stops the request without sending it`() {
        val flight = PluginFetch.Flight()
        val transport = Recorder(emptyMap())
        val outcome = exchange(
            grants("fetch"),
    ...
    @Test
    fun `an abort mid-chain stops the redirects rather than following twenty more`() {
    ...
    @Test
    fun `the body of a redirect that was followed is deleted and its bytes come back`() {
        val budget = AtomicLong(0)
     ...
     */
    @Test
    fun `a chain of redirects that all carry bodies leaves nothing behind`() {
        val budget = AtomicLong(0)
    ...
    @Test
    fun `discarding the same hop twice does not credit it twice`() {
        val budget = AtomicLong(0)
    ...
    @Test
    fun `an answer no engine is waiting for is dropped and its bytes come back`() {
        val budget = AtomicLong(0)
    ...
    @Test
    fun `an answer the engine took keeps its body and its charge`() {
        val budget = AtomicLong(0)
        val kept = bodyHop(200, null, 900, budget)
     ...
     */
    @Test
    fun `an answer for a request the plugin aborted is dropped rather than settled`() {
        val budget = AtomicLong(0)
        val kept = bodyHop(200, null, 900, budget)
    ...
    @Test
    fun `each install holds its own budget and wiping one drops it`() {
    ...
    @Test
    fun `a body past the per-response ceiling is refused while it is being read`() {
        val budget = AtomicLong(0)
    ...
    @Test
    fun `a body past what this plugin already holds is refused against the budget's own number`() {
    ...
    @Test
    fun `a body inside both ceilings is charged exactly what it wrote`() {
        val budget = AtomicLong(0)
    ...
    @Test
    fun `a body that was cancelled mid-read leaves nothing charged`() {
        val budget = AtomicLong(0)
        val flight = PluginFetch.Flight()
        flight.cancel()
        val file = File.createTempFile("inu-drain", ".bin").apply { deleteOnExit() }
     ...
     */
    @Test
    fun `the two body ceilings are the numbers the contract states`() {
        val mb = 1024L * 1024L
     ...
     */
    @Test
    fun `a connection this api opens never follows a redirect on its own`() {
