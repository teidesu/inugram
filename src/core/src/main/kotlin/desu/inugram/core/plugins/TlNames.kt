 ...
 *
 * the name is whatever the TL schema calls the constructor, so [TL_NAME_OVERRIDES] lists every class
 * stock spells differently. the exception is the legacy variants of one predicate: `TL_message` and
 * `TL_message_old7` are both `message` on the wire, so whichever loses the name keeps its derived
 * one.
 *
