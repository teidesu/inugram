...
type Disposing = crate::testing::util::DisposeOnDrop<NotificationState>;
type Fixture = (Runtime, Context, Rc<TestNotificationHost>, Disposing, std::sync::Arc<crate::testing::util::Logs>);
...
fn setup(grants: &[&str]) -> Fixture {
    ...
    let state = ctx.with(|ctx| {
        ...
        install_notifications(&ctx, host_dyn, grants, Lifecycle::new(), log.clone()).unwrap()
    });
...
fn the_bundled_notifications_test_plugin_passes() {
    let (rt, ctx, host, state, logs) = setup(&crate::testing::util::manifest_grants(ORACLE));
    ...
    crate::testing::util::assert_oracle_exact(&lines, "notifications test done", 12);
    assert!(logs.borrow().is_empty(), "unexpected logs: {:?}", logs.borrow());
}
