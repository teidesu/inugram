...
type Disposing = crate::testing::util::DisposeOnDrop<RpcState>;

type LoggingFixture = (Runtime, Context, Rc<TestHost>, Disposing, std::sync::Arc<crate::testing::util::Logs>);
...
fn setup_logging(grants: &[&str]) -> LoggingFixture {
    ...
    let accounts_host: Rc<dyn crate::tg::account::AccountHost> =
        crate::tg::account::tests::TestAccountHost::with(crate::tg::account::tests::TWO_ACCOUNTS);
...
fn invoke_rpc_resolves_and_rejects() {
    ...
    ctx.with(|ctx| {
        ctx.eval::<(), _>(r#"inu.invokeRpc({_:'foo.baz'}).catch(e => { globalThis.__err = e.message; });"#).unwrap();
    });
...
fn levels(logs: &std::sync::Arc<crate::testing::util::Logs>) -> Vec<(i32, String)> {
    logs.borrow()
        .iter()
        .map(|line| {
...
fn a_plugin_throw_faults_where_a_host_failure_does_not() {
    ...
    install_rejection_tracker(&rt, crate::testing::util::log_sink(&logs));
    eval(
        &ctx,
        r#"
...
fn a_spinning_middleware_is_interrupted_and_the_request_is_still_answered() {
    let (rt, ctx, host, state, logs) = setup_logging(&["interceptRpc"]);
    crate::engine::deadline::install_interrupt_handler(&rt, std::sync::Arc::new(|_: &str| {}));
    ctx.with(|ctx| {
        ctx.eval::<(), _>("inu.interceptRpc('foo.bar', (req, next) => { while (true) {} });").unwrap();
    });

    {
        let _deadline = crate::engine::deadline::arm(50);
...
fn a_demuxed_registration_names_its_own_constructors_and_its_own_grant_scope() {
    ...
    assert_eq!(
        seen,
        vec![
            (vec!["updateNewMessage".to_string(), "updateNewChannelMessage".to_string()], "new_message".to_string()),
            (vec!["updateEditMessage".to_string(), "updateEditChannelMessage".to_string()], "edit_message".to_string()),
            (
...
fn the_bundled_events_test_plugin_passes() {
    ...
    let (rt, ctx, _host, state) = setup(&crate::testing::util::manifest_grants(ORACLE));
    ...
    for (type_name, wire) in [
        ...
        ("updateDeleteChannelMessages", r#"J{"_":"updateDeleteChannelMessages","channel_id":"99","messages":[43]}"#),
    ...
    crate::testing::util::assert_oracle_exact(&lines, "events test done", 10);
}
...
fn the_bundled_accounts_test_plugin_passes() {
    ...
    let host_dyn: Rc<dyn RpcHost> = host.clone();
    let tl = TlViews::new(host.clone());
    let grant_host = TestGrantHost::new(&crate::testing::util::manifest_grants(ORACLE));
    let log: crate::Log = std::sync::Arc::new(|_: &str| {});
    let accounts_host = crate::tg::account::tests::TestAccountHost::with(crate::tg::account::tests::TWO_ACCOUNTS);
    let accounts_dyn: Rc<dyn crate::tg::account::AccountHost> = accounts_host.clone();
    ...
    let _accounts_disposer =
        crate::testing::util::DisposeOnDrop::new(&ctx, accounts.clone(), crate::tg::account::dispose);
    let state = Disposing::new(&ctx, state, dispose);
    ...
    *accounts_host.json.borrow_mut() = SWITCHED.to_string();
    crate::tg::account::accounts_changed(&rt, &ctx, &accounts);
    dispatch_update(
        &rt,
        &ctx,
        &state,
    ...
    crate::testing::util::assert_oracle_exact(&lines, "accounts test done", 15);
}
...
fn a_send_disposer_takes_the_stage_out_of_the_chain() {
    let (rt, ctx, host, state) = setup(&["interceptSendMessage"]);
    eval(&ctx, "globalThis.__ran = 0; globalThis.__d = inu.interceptSendMessage(() => { __ran++; return 'drop' });");
    let callback_id = send_callback_id(&host);
    eval(&ctx, "__d(); __d();");
...
fn the_bundled_send_intercept_test_plugin_passes() {
    ...
    let (rt, ctx, host, state) = setup(&crate::testing::util::manifest_grants(ORACLE));
    let lines = crate::testing::util::install_capturing_console(&ctx);
    ctx.with(|ctx| match ctx.eval::<(), _>(ORACLE) {
        Ok(()) => {}
        Err(rquickjs::Error::Exception) => panic!("{}", format_exception(&ctx)),
    ...
    let lines = lines.borrow();
    crate::testing::util::assert_oracle_exact(&lines, "send intercept test done", 27);
}
...
fn the_bundled_update_intercept_test_plugin_passes() {
    ...
    let lines = lines.borrow();
    crate::testing::util::assert_oracle_exact(&lines, "update intercept test done", 16);
}
...
fn serde_verdicts(host: &Rc<TestHost>) -> String {
    let verdicts: Vec<String> =
        host.verdicts.borrow().iter().map(|(id, deliver)| format!("[{id},{deliver}]")).collect();
...
/// [`crate::testing::util::assert_oracle_exact`]'s exact count is for.
#[cfg(test)]
mod bundled_oracles {
    use super::*;
    use crate::engine::error::TestGrantHost;
    ...
    impl TlHost for OracleHost {
        ...
        fn tl_own_keys(&self, handle: i64) -> Option<String> {
            ...
            let keys: Vec<String> = match &*node.borrow() {
                Node::Object { fields, .. } => {
                    fields.iter().map(|(k, _)| k.clone()).filter(|k| !hidden.contains(k)).collect()
                }
        ...
        fn on_next(&self, dispatch_id: i64, request_wire: &str) -> Option<String> {
            ...
            if !expected.is_empty() && method != expected {
                return Some(plugin_error(
                    "forbidden",
                    &format!("next(): expected a '{expected}' request, got '{method}'"),
                ));
            }
    ...
    type Disposing = crate::testing::util::DisposeOnDrop<RpcState>;
    type Fixture = (Runtime, Context, Rc<OracleHost>, Disposing, std::sync::Arc<crate::testing::util::Logs>);
    ...
    fn setup(grants: &[&str], with_globals: bool) -> Fixture {
        ...
        let state = ctx.with(|ctx| {
            ...
            if with_globals {
                ...
                crate::engine::globals::install_globals(
                    &ctx,
                    random,
                    std::path::Path::new(""),
    ...
    fn the_bundled_globals_test_plugin_passes() {
        let (rt, ctx, host, state, _logs) = setup(&crate::testing::util::manifest_grants(GLOBALS_ORACLE), true);
        let lines = crate::testing::util::install_capturing_console(&ctx);
        ...
        let lines = lines.borrow();
        crate::testing::util::assert_oracle_exact(&lines, "globals test done", 38);
    }

    #[test]
    fn the_bundled_lazy_tl_test_plugin_passes() {
        let (rt, ctx, host, state, _logs) = setup(&crate::testing::util::manifest_grants(LAZY_TL_ORACLE), false);
        let lines = crate::testing::util::install_capturing_console(&ctx);
        ...
        let lines = lines.borrow();
        crate::testing::util::assert_oracle_exact(&lines, "lazy tl test done", 10);
    }
    ...
    fn the_bundled_takeover_test_plugin_passes() {
        let (rt, ctx, host, state, _logs) = setup(&crate::testing::util::manifest_grants(TAKEOVER_ORACLE), false);
        ...
        *host.chain_method.borrow_mut() = "help.getConfig".to_string();
        ...
        let lines = lines.borrow();
        crate::testing::util::assert_oracle_exact(&lines, "takeover test done", 6);
    }
    ...
    fn the_bundled_api_filter_test_plugin_passes() {
        let (rt, ctx, host, state, _logs) = setup(&crate::testing::util::manifest_grants(API_FILTER_ORACLE), false);
        ...
        let lines = lines.borrow();
        crate::testing::util::assert_oracle_exact(&lines, "api filter test done", 3);
    }
