...
//! `InputPeerLike` resolver [`crate::tl::utils`] hands [`crate::tg::reads`].
//!
...
use crate::engine::error::{check_grant, get_or_create_inu, wire_error_to_js, GrantHost, MATCH_EXACT};
use crate::io::blob::BlobState;
use crate::tg::account::AccountState;
use crate::tg::progress::ProgressReporter;
...
fn throw_write<'js, T>(ctx: &Ctx<'js>, code: &str, message: &str) -> JsResult<T> {
    crate::engine::error::throw_plugin_error(ctx, code, message, None, None, None)
}
...
fn js_write<'js>(
    ...
    let outcome = (|| -> JsResult<()> {
        for value in crate::engine::argv::array_values(ctx, &values, "account write")? {
