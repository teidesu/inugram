...
pub(crate) type Disposing = crate::testing::util::DisposeOnDrop<ActionState>;
pub(crate) type Fixture = (Runtime, Context, Rc<TestActionHost>, Disposing, std::sync::Arc<crate::testing::util::Logs>);
...
fn setup_with(lifecycle: Rc<Lifecycle>, grants: &[&str]) -> Fixture {
    ...
    let state = ctx.with(|ctx| {
        ...
        let grants = crate::engine::error::TestGrantHost::new(grants).as_host();
...
fn a_callback_that_throws_is_the_plugins_fault() {
    let (rt, ctx, _host, state, logs) = setup();
    eval(&ctx, r#"inu.registerChatAction({ id: 'a', text: 'A', callback: () => { throw new Error('nope') } })"#);
...
fn kinds_are_separate_registries() {
    ...
    assert_eq!(render_actions(&rt, &ctx, &state, KIND_MESSAGE, MESSAGE_SURFACE).unwrap(), rows(&[row(1, "message")]));
}
...
fn a_bad_editor_argument_is_an_invalid_argument_error() {
    ...
    dispatch_action(
        ...
        KIND_EDITOR,
        1,
        r#"{"accountId":0,"dialogId":5,"surface":1,"draft":{"text":""}}"#,
    );
...
mod bundled_oracle {
    ...
    fn the_bundled_actions_test_plugin_passes() {
        ...
        crate::testing::util::assert_oracle_exact(&lines, "actions test done", ORACLE_ASSERTIONS);
