...
type Disposing = crate::testing::util::DisposeOnDrop<TimerState>;
type Fixture =
    (Runtime, Context, Rc<TestTimerHost>, Rc<Lifecycle>, Disposing, std::sync::Arc<crate::testing::util::Logs>);
...
fn an_interval_asking_for_zero_repeats_at_the_floor_the_contract_states() {
    let floor = crate::testing::util::stated_number(
...
///
/// It needs three things at once - the wheel here, `inu.kv`/`inu.onUnload` from [`crate::api`], and
...
mod bundled_oracle {
    use super::*;
    use crate::api::{ApiHost, KV_DEL, KV_GET, KV_SET};
    ...
    /// `src/kotlin/helpers/plugins/PluginApi.kt` owes a test. Everything else the oracle asserts is
    /// the wheel's, and that is here.
    const DUTY_PERCENT: u128 = 10;
    const MIN_WAKE_GAP_MS: u64 = 4;

    #[derive(Default)]
    struct OracleApiHost {
        store: RefCell<BTreeMap<String, String>>,
    }

    impl ApiHost for OracleApiHost {
        fn kv(&self, op: i32, key: &str, value: &str) -> String {
            ...
            match op {
                ...
                _ => crate::tl::proxy::encode_error("the oracle asks for nothing else"),
            }
        }
    ...
    fn the_bundled_timers_test_plugin_passes() {
        ...
        let api_host: Rc<dyn ApiHost> = Rc::new(OracleApiHost::default());
        ...
        let (api, timers) = ctx.with(|ctx| {
            ...
            let api = crate::api::install_api(&ctx, api_host, grants, lifecycle.clone(), log.clone()).unwrap();
        ...
        crate::api::notify_unload(&rt, &ctx, &api);
        crate::api::dispose(&ctx, &api);
        dispose(&ctx, &timers);

        let lines = lines.borrow();
        crate::testing::util::assert_oracle_exact(&lines, "timers test done", 5);
    }
    ...
    fn set_app_visible(
        rt: &Runtime,
        ctx: &Context,
        api: &Rc<crate::api::ApiState>,
        timers: &Rc<TimerState>,
        visible: bool,
    ) {
        set_visible(timers, visible);
        crate::api::app_visibility_changed(rt, ctx, api, visible);
    }
    ...
    fn the_bundled_visibility_test_plugin_passes() {
        ...
        let wakes_dyn: Rc<dyn TimerHost> = wakes.clone();
        let api_host: Rc<dyn ApiHost> = Rc::new(OracleApiHost::default());
        let grants = TestGrantHost::new(&crate::testing::util::manifest_grants(VISIBILITY_ORACLE)).as_host();
        let log: crate::Log = std::sync::Arc::new(|_| {});
        ...
        let lines = lines.borrow();
        crate::testing::util::assert_oracle_exact(&lines, "visibility test done", 12);
    }
}
