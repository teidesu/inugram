...
fn a_spinning_microtask_is_interrupted() {
    ...
    crate::tg::rpc::pump_jobs(&rt, &ctx, log.as_ref());
...
mod memory_tests {
    ...
    fn eval(ctx: &Context, code: &str) -> Result<String, String> {
        ctx.with(|ctx| match ctx.eval::<Value, _>(code) {
            ...
            Err(rquickjs::Error::Exception) => Err(crate::tg::rpc::format_exception(&ctx)),
...
mod entry_points {
    ...
    fn every_jni_export_arms_a_deadline() {
        ...
        for (index, _) in source.match_indices(PREFIX) {
            ...
            assert!(
                body[..end].contains("deadline::arm_entry_deadline()")
                    || body[..end].contains("deadline::arm_eval_deadline()"),
