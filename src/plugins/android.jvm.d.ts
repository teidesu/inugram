...
declare namespace inu {
  ...
  namespace jvm {
     ...
     *
     * **Note**: JS always runs on `globalQueue`, and the Runnable generated by this function will post
     * to it if it's on another thread. If you need to run some code on an an explicit thread, use
     * `inu.jvm.routine` instead.
     */
