...
- Native API code lives in `src/native/src/api`, JNI in `jni`, grants/limits in
  ...
  thin LSPlant ABI wrappers in patches. The release profile builds the engine for size and
  overrides hot crates for speed (`yuv`, QuickJS's C code); put per-pixel work
  in such a crate and add it to those overrides.
...
- A send takes rust's staged transfer by rename (`PluginTransfers` stages on the
  media cache volume) and uploads a plugin's `{ path }` in place, never moving or
  deleting it. `PluginSentFiles` moves a sent host-owned gif to its attach path.
