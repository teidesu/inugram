...
- TL views share one proxy handler per context; the target `HandleBox` carries the view's state and its cache. A handle minted for a plugin's own read carries its scalar fields (`HOR12|{...}`, `TlHandles.projectScalars`) and rust seeds the cache with them. The projection and `getObjectField` must answer a field identically; `TlProjectionTest` checks that against real objects.
- A projection also carries a field whose object is nothing but scalars (`TlReflect.isFullyScalar` - a peer, an inputPeer), as `{"@h":"OR12",...}`: rust builds that child view while it adopts the parent, so the child owns its handle from that moment and releases it like any other. Every other child handle crosses with its type name alone, `_` being the field a plugin reads off a nested object most. A dispatch-scoped handle carries neither, because that view caches nothing.
- The projection is assembled into a `StringBuilder`, and the pieces it assembles are quoted by `JSONObject.quote`, cached per class and per field. Measured on a Pixel 9, 200 dialogs: building it with `JSONObject` instead costs 68.7us per object against 26.6us, and quoting a name per object rather than caching it costs ~1.5us a call, ~25 calls deep. What quickjs then charges to parse one is per property, not per byte (15.5us for ~20 properties, 2.1us for the same bytes under one key), so a denser wire format buys ~2us and a shorter projection buys the rest.
