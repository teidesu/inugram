...
The host half is `src/kotlin/helpers/plugins`, grouped to mirror the crate: `telegram/` (`PluginRpc`,
...
(`nativeCreate()` in its constructor, `System.loadLibrary` in its class initializer), so the
...
excluded-files list for whatever needed a `Context`; every one of its cases either moved here or
turned out not to be a bridge test at all. **A change to any rule below without a case on the device
is how the last three of them were broken.**

**Where a test goes is decided by one rule.** It is: **a fake must never be the source of a truth the
test asserts.** That rule is what emptied the harness — a fake standing in for the platform or for
stock (storage semantics, TL shape, `org.json`'s ordering, the notification-event vocabulary,
okhttp's header handling, what a dex loader accepts) drifts silently, and a fake supplying a
*stimulus the test itself chose* is always cheaper to rebuild on a device than to keep. The clock is
...
sources` (`StockHooksTest` for the stock java hooks, `PluginBootTest` for the boot sequence,
`ForkWiringTest` for wiring in files nothing compiles and for constants rust and Kotlin each drive
their own tests off) plus the cargo test parsing `PluginListener.kt`. They are lints over the
patchset, run by `:InuCore:check` with the pure-core suite, and they need no fake at all. **One
assertion was genuinely lost** with the harness and is a lint now: that a guid bound before the
chain armed is re-applied behind the passthrough. `native_bindRequestToGuid` is `public static
native`, so observing the call means shadowing `ConnectionsManager`, which the device cannot do.

Three consequences worth knowing before writing on the device side. The bridge lives in the app's
own process, so a double cannot *shadow* a class — which is why
`QuickJs` is `open` (for `RecordingQuickJs`, its only subclass anywhere) and why a stock singleton
whose constructor opens sockets is built with `sun.misc.Unsafe.allocateInstance` and no constructor
at all (`TestApp.allocate`), leaving every field it would have set at zero. The queues are swapped
into `Utilities.globalQueue`/`stageQueue`/`cacheClearQueue` (all `volatile`, none `final`) and
**every recorder is installed eagerly in `resetBridge`**, because a lazily installed one is absent
until a test first asks for it and a send before that reaches the app's real manager and the network
behind it. And `:TMessagesProj` is a *library* module, so its androidTest apk is standalone rather
than instrumenting the app: nothing puts the app module's jniLibs in it, and `QuickJs`'s class
initializer loads `libinu_native.so`, so `sourceSets.androidTest.jniLibs` points at
`TMessagesProj_App/build/rustJniLibs` and the merge task depends on `:TMessagesProj_App:buildInuNative`.

Three more rules the device imposes: a test method
name may not contain **spaces** (dex forbids them below API 30 and `minSdk` is 26, so these are
`snake_case` like the crate's own tests), the runner is scoped to `desu.inugram` in `defaultConfig`
(bouncycastle ships test classes inside a jar the app depends on, and 21 of them get swept up
otherwise, 2 failing on their own), and **anything keyed by install id is real storage that
outlives the test that wrote it** — hence `freshInstallId()` per test, since `apply()` reaches disk
asynchronously and a directory sweep misses a store still only in the cache. `startPlugin` is the
exception and the trap: its id is the plugin *name*'s hash, so two tests naming one share a
directory and the second stands on the first's files, which is why `PluginJvmTest` wipes the dex
root in `setUp`. The contract `.d.ts` files, the bundled oracles and `probe.dex` ride along as test
assets, because `statedNumber`, the suites that read a plugin's own `@grant` line and `loadDex` have
no repo to read on a device — the first two from `src/plugins`, the last from `src/androidTestAssets`,
which exists because `src/androidTest` itself is synced into a *kotlin source root*.

**The app is *running* around the test, and that is the device's one genuinely hard rule.** The real
tgnet library is loaded, so stock's own `ConnectionsManager.onUpdate` callback drives
`MessagesController.updateTimerProc` from stock's own thread and sends `help.getPromoData`,
`help.getTermsOfServiceUpdate` and `account.updateStatus` at moments no test controls. Recorded,
those land in `RecordingConnectionsManager.sent` beside the request the test is about, and
`lastSent()` then answers the wrong one — a flake in whichever suite happened to be running, not in
the one that caused it. So work stock posts from stock's own threads is dropped, at both boundaries
it can arrive through: `TestQueues.post` (a runnable not from `desu.inugram.*`, posted from neither
the test thread nor the ui thread) and `RecordingConnectionsManager.sendRequestInternal` (a send off
the test thread). Both halves of the queue rule are load-bearing — the bridge posts there from
threads a test does not run on, `interceptDeserialize`'s middleware parking the parsing thread on
exactly such a post, and a test drives the ui thread deliberately through `runOnMainSync`. Two
**Its native threads are running too, and those abort the process rather than failing a test.**
`libtmessages`'s own connection thread calls back into java the whole time, and under CheckJNI an
exception pending on the way out is a `JniAbort` - which arrives as an *empty* `<failure>` on
whichever test was running, in a report the next green run overwrites. Two rules come out of it.
Native is told there is no network at all in `resetBridge` (`goOffline`), because a response stock
cannot parse aborts inside `ConnectionsManager::select` and nothing java-side is on that path. And
**a stock singleton built by `TestApp.allocate` must have every container a native callback reads**:
`onRequestComplete` reads `requestCallbacks`, which `Unsafe.allocateInstance` leaves null, so
`RecordingConnectionsManager.fillContainers` gives every plain container the class declares the
empty value its initializer would have. That claims nothing about stock - an empty container is what
a callback would have found before the test sent anything - and it is the shape any future recorder
of a natively-driven class needs. Two smaller consequences of the same fact:
`AndroidUtilities.runOnUIThread` genuinely defers, so
anything reaching the app through it (`PluginMedia`'s `loadFile`/`uploadFile`) needs `settle()` and
not `drain()`; and a `TLObject` subclass a test writes to count `freeResources()` is named by
`TlNames.classNameToTlName` off **its own class**, so a counter over a request must keep the stock
name (`PluginRpcChainTest.TL_users_getUsers`) or it enters a chain registered for another method.

**What the move caught, all of it green in the harness beforehand:** the fake `NotificationCenter`
invented 8 event ids while `PluginNotifications` reflects over the real class's ~400, and knows
nothing of stock *debouncing* `updateInterfaces` by 250 ms (`shouldDebounce`); the fake
`processUpdates` appended the batch to a list while the real one **removes entries from
`updates.updates` in place**, so `PluginWrites.send` read the sent message back out of an emptied
list and *every* plugin `sendMessage` failed on a real device — it snapshots the list around the
hand-back now, and the emptied original is what the test asserts on; the fake put `TLObject`'s
`disableFree` check on the base class, where stock's is empty and the check lives in whichever
subclass owns a `NativeByteBuffer`; the fake `DexClassLoader` took four bytes and recorded a path,
where the platform's verifies the file, refuses to map a *writable* one as code whoever wrote it,
and leaves its own compiled artifacts beside it; and `inu_freeCount` and the fake's public
`MessagesController.users`/`chats` were API stock does not have at all.

**A JNI export's name carries the package of the class it implements**, so moving that class into a
subpackage silently unbinds every one of its natives: the app throws `UnsatisfiedLinkError` at the
first call and nothing before launch says a word. `PluginXposed` moved into `platform/` and its six
exports kept the old name, which left `inu.xposed` dead on every device — and the one test that
enumerated exports had the broken names baked into its exempt list. `jni/tests.rs` now pins it both
ways (`every_jni_export_names_the_package_its_kotlin_actually_lives_in`,
`every_kotlin_native_has_a_rust_export`) against the Kotlin sources. This is the same hazard that
keeps `QuickJs` at the root, applied to a class that did move.
...
- **A runaway plugin can only be stopped from inside quickjs.** Every engine op shares `globalQueue`
  ...
  `sandbox/limits.rs`: every JNI export that can run plugin JS arms a thread-local deadline (2 s per entry,
...
- **Memory is bounded per engine, in two counters that cannot see each other.** `sandbox/limits.rs` also
...
- **Every `on*`/`intercept*`/`register*` returns a `Disposer`, and they all behave identically**:
  ...
  disposer. `sandbox/registry.rs` holds all of that once; do not hand-roll a second mechanism. Every
...
- **The async reads are the same materialization, one queue hop later.** `getHistory`/`getDialogs`/
  ...
  pinned by a test rather than inherited on faith. Two rules they add: a peer is resolved
...
- **`inu.jvm` hands over the app, so the scope list is the only boundary and every crossing checks
  ...
  is why the whole of it is Kotlin and tested on a device. Two rules are call-time rather than scope
