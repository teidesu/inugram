...
- The message reads are two calls because the app has two answers. `getMessagesCached` is synchronous and reads `MessagesController.dialogMessage`/`dialogMessagesByIds`, which hold **only the chat list's own last message per dialog** - a message the user is looking at belongs to `ChatActivity`'s own dict and is not in either, so a miss there says nothing. `getMessages` is async and walks memory -> `messages_v2` on the storage queue -> `messages.getMessages`/`channels.getMessages`, narrowing the id list at each step and reaching exactly one `answer`. Both take one id or a list, and answer in that shape.
- `0` is a dialog id nothing has, and the message reads give it a meaning: the **common message box**, the one id sequence telegram numbers every user chat and basic group out of, so an id from one of those needs no peer. It crosses as the ordinary spec `D0` - there is no vocabulary for it - and `toSpec` accepts it like any other id, so every *other* surface answers the miss it is (`getUser(0)` is `null`, `sendMessage(0, ...)` is `not-found`) rather than the `invalid-argument` it used to refuse with. A channel numbers its own messages, so `dialogMessagesByIds` (keyed by the bare id) and a common-box storage read (`is_channel = 0`) must both refuse a channel message rather than hand back whichever one collides on that number.

- A write has two paths and `optimistic` picks between them. On by default, `sendMessage` and `sendMedia` go through `SendMessagesHelper` the way the composer does: a local message is drawn before anything goes out, stock uploads from the staged path and reports its progress on the bubble, and a failure is retryable from the chat. `optimistic: false` keeps the request `PluginWrites.send` builds, and so does anything the composer cannot say - a `sendAs` peer (a local message's sender is the dialog's own default), a file that is already an `InputFile`/`InputMedia`, every album, and a `replyToMessageId` naming a message the app does not already hold. **The promise resolves with the server's message either way**; only what the user sees in the meantime differs. Two things the composer does that the request path does not: it clears the dialog's draft, and it recompresses an image into a photo unless `asDocument` says otherwise.
- `interceptSendMessage` runs at `ConnectionsManager.sendRequest`, the last step of a send, so a `drop` used to arrive with the bubble already on screen and nothing left to do but delete it. `PluginSendHold` parks stock's own draw (the `updateInterfaceWithMessages` + `dialogsNeedReload` pair inside `sendMessage`) for **at most 100ms**, released by whichever comes first: the chain passing the request through (`sendPassthrough`, behind the text rewrite, which took the same ui hop), a middleware dropping it, `bindOptimisticMessages` finding no interceptor that matches, `maybeIntercept` declining the request, or the deadline. Nothing about the send is delayed - the request goes out while its bubble is parked - and the deadline means a slow or hung middleware degrades to exactly the old flash. The park is decided from the text alone, through `SendFilter.matchesSend`: everything a send filter reads except the method is known before the request exists, so a filtered middleware costs only the messages it could actually claim and an account with no `interceptSendMessage` registration pays nothing at all. A middleware nothing could wait for parks nothing either: rust reads `constructor.name == "AsyncFunction"` off the callback at registration and carries `deferred` in the registration json (which is why that json is sent for an unfiltered registration at all), and a deferred interceptor is skipped by `maySendBeIntercepted`. Registration is the only moment this is decidable without a new host callback, so it catches an `async` middleware and not a plain function that happens to return a promise; the latter still waits out the deadline. A send parked in a dialog holds back the sends behind it in that dialog, or the two would arrive out of order. Media rarely benefits, its request being built only once the upload finishes.
- What a middleware answers a chain with is one of two kinds, and they are not carried the same way. A **response** is what the request produced, real or fabricated, a success or a `FLOOD_WAIT_5`, and it is every outer stage's to catch and substitute - that is what middleware is for. A **verdict** is the decision that this request is not happening at all: `drop`, and the takeover `message.setMedia()` asks for. A verdict is *not* substitutable, because by the time one is made the app already owes a local message an unwind, so it is typed onto the chain (`ChainVerdict`, `chainVerdicts[scopeId]`) where it is decided and read by the top-level `finalize`, the one place that answers the app. A drop is authored in js, so it crosses as an `RpcError` on the synthetic code and is read **once**, in `onComplete`, and only from a stage whose scope is `interceptSendMessage`; a takeover is decided in kotlin at the chain end and never crosses at all. `finalize` then mints the `TL_error` the composer's error path needs, because a request delegate has no other vocabulary for "this send did not happen" - but nothing downstream reads its text. `handleDroppedSend`/`handleDroppedEdit` consult `sendVerdicts`, keyed by account *and* local message id, since every account mints its local ids out of the same descending sequence. So a stage above the deciding one cannot revoke a verdict by catching what its `next()` rejected with, and a plugin cannot forge one by returning an error that reads like it. A verdict also owes the dialog's draft: stock cleared it locally the moment it handed the send to the network (`SendMessagesHelper`, right after `performSendMessageRequest`, which only *posts* `sendRequestInternal`), but the server's own copy rides the request's `clear_draft` flag, and that request is the one that never goes out - a takeover's re-send carries `clear_draft = false` too, stock reading a retry as a send whose draft went with the first attempt. So `bindOptimisticMessages` notes the draft's key while it can still see one (it runs inside `performSendMessageRequest`, a few lines before the clean) and `finalize` sends the empty `saveDraft` itself. Without it the command the user typed comes back into the composer on the next draft sync.
- **A verdict arrives after the composer has already committed every step of a send, and each one needs a counterpart.** `SendMessagesHelper.sendMessage` mints the local message and runs a row of side effects well before `sendRequestInternal` is reached, so an unwind has to undo them rather than merely stop the request. What the real path does, and what answers it today:
  - `send_state = MESSAGE_SEND_STATE_SENDING` plus `MessagesStorage.putMessages` - the local message is **persisted**, not only drawn. A drop's `deleteMessages` takes it out of both; a takeover's re-send writes the same id again.
  - the draw itself (`PluginSendHold.draw`, standing in for stock's `updateInterfaceWithMessages` + `dialogsNeedReload`) - parked by `PluginSendHold`, and turned into a `replaceMessagesObjects` by `PluginSendMorph.redrawInstead`.
  - `putToSendingMessages` and the `sendingMessages` map - `processSentMessage` + `removeFromSendingMessages`, which both verdict paths call.
  - `cleanDraft`, which is local only - the server's copy rides `clear_draft` on the request, so `finalize` sends the empty `saveDraft` (above).
  - the composer's 200ms text timer, which `ChatActivity` normally cuts short by `startMessageTransition` when the bubble arrives - `PluginSendHold.finishSendTransition`, because a send with no bubble has nothing to cut it short.
  - `StarsController.beforeSendingMessage`, for a paid dialog - **nothing undoes this**, and whether a verdict owes it anything has not been checked.
  - media: `delayedMessages` and the upload are registered before the request exists at all, so a verdict on a media send lands after the file has already gone up.

  Each of these is the same shape: the real send path does the work, the verdict path reuses the composer's *error* seam, and whatever that seam does not already unwind is missed in silence. Two of them (the text timer, the draft) were found only after a user saw them. Enumerate what the real path does before reusing a seam, rather than fixing instances as they are reported.
- The composer answers through `NotificationCenter`, not a delegate, so a send is followed by a token in `Message.params` - which stock persists and hands back on every retry of the same message. A reply is resolved to the real message first (`PluginReads.loadLocalMessages`: memory, then `messages_v2`, never the network), because `MessageObject` takes what it is handed as its own `replyMessageObject` and that is the one thing that stops `ChatActivity` looking the real message up - a stub would draw a quote with a name and no text until the chat was reopened. The observer is **per send**, added when the write starts and removed when it settles: a plugin's listener may not outlive the plugin, and nothing else says when one of these is done. The request the composer eventually builds takes the same bypass lease `sendWithoutInterceptors` takes, claimed in `PluginRpc.bindOptimisticMessages` (which is already called with the `MessageObject` in hand, one line before `sendRequest`) and released when the send settles or the plugin detaches - so a plugin's own send still never re-enters a send interceptor, its own included.
...
- A canvas draw is recorded and replayed in a batch, so what a plugin actually pays per call is `measureText`, which crosses on its own and which a wrap loop runs once per candidate line. Its cost is the font: resolving a family reaches `FontLibrary.getTypefaceByName`, whose roster entries answer their display name through `LocaleController`, at ~310us a lookup. `PluginCanvas.Session` therefore caches the typeface per font wire (everything but the size) and drops it when `loadFont` adds a family or `FontLibrary.rosterGeneration` moves, which took a `measureText` from 864us to 82us and 200 text draws from 81ms to 12ms on a Pixel 9 Pro XL. `CanvasBenchTest` measures the three layers separately - js, the listener alone, the platform calls alone - so a slow op says which one it is.
- What is left in a render is the platform's own: decoding the source bitmap and `Bitmap.compress`, tens of milliseconds each for a photo, against ~5ms for the whole measure and draw pass. Neither is the bridge and neither is worth shaving crossings for.
