...
- Android plugins can define JVM subclasses and interface implementations with instance/static fields, constructors, and JS or interpreted-routine method bodies (`inu.jvm.defineClass`); the host names the class unless one is asked for, since a loaded dex stays loaded and a name of the plugin's own can never be defined again
