...
- importing a plugin that matches one already installed (same `@name` + `@author`) updates it in place instead of installing a second copy: same storage, same position, same enabled state, confirmed by a sheet showing the version change and only the permissions it newly asks for; an identical file is refused with an "already installed" bulletin
