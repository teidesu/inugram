...
- install flow: tapping a `.inu.js` file in chat (or picking one from disk) opens a confirmation sheet with the plugin's name, description, requested permissions, obfuscation/minification warnings, a source code viewer and an "enable after installing" toggle; plugins this app can't run are refused with a readable reason
