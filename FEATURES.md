...
- plugins can rewrite or cancel outgoing messages (`inu.interceptSendMessage`), with optional native text/edit filters, and rewrite or drop incoming updates before the app applies them (`inu.interceptUpdate`); a verdict within 100ms can cancel without showing a bubble, including async validation; longer work shows a pending bubble that can grow into the generated media or be removed when the chain finishes
- plugins can read an animated source frame by frame and write a video back: `inu.canvas.decodeAnimation()` opens a gif, mp4, webm (video stickers included) or lottie sticker through the app's own decoders, decodes it at the size asked for, and is iterated with `for await` for each frame as an `ImageBitmap` carrying its timestamp (or indexed with `frame(i)`, counting actual frames even with variable delays); `inu.canvas.createEncoder()` takes frames from a canvas or an image and writes a silent mp4, preserving every frame's duration and charging queued pixels against the plugin's memory limit, so sending one arrives as a gif rather than as a video file
- a plugin's `sendMedia` describes a video the way the composer does (dimensions, duration, streaming, a silent one as an animation, a first-frame cover), so an mp4 it sends plays in place instead of arriving as a file; the copy it uploaded from is moved to where a download of it would land once the server answers, the way stock does for its own videos
- plugins can list the fonts the app itself has and be handed a file by the user: `inu.canvas.listFonts()` answers every family a `ctx.font` resolves - bundled, imported, the device's own, and the plugin's own loads - with where each came from, and `inu.ui.pickFile()`/`inu.ui.saveFile()` open the device's document picker, the gesture being the permission: a picked file arrives as a `File` copied into the plugin's storage and no path of the device's is reported either way
- plugins page in settings: engine toggle, install from file, enable/disable, reload, share, drag-handle reorder (middleware order); roomy list by default (description + settings/reload/remove actions per plugin), compact single-row view via the overflow menu; a plugin that crashes is switched off with a bulletin naming it and the error, whose View button opens its info page
- install flow: tapping a `.inu.js` file in chat (or picking one from disk) opens a confirmation sheet with the plugin's name, description, requested permissions (worst first, dangerous ones explained), obfuscation/minification warnings, a source code viewer and an "enable after installing" toggle; the confirm button scrolls until the permissions have been seen, and installing can be undone from the bulletin; plugins this app can't run are refused with a readable reason
- importing a plugin that matches one already installed (same `@id`, or the same `@name` + `@author` where the file declares none) updates it in place instead of installing a second copy: same storage, same position, same enabled state, confirmed by a sheet showing the version change, the permissions it newly asks for, the ones it gives up (dimmed), and a button revealing the ones it already holds; an identical file is refused with an "already installed" bulletin
...
- per-plugin info page (tap a plugin row): name, author, version, localized description, requested permissions (user-readable descriptions, color-coded by danger tier), syntax-highlighted source code viewer, enable/disable, reload/share/remove; warns when the plugin code looks obfuscated or minified. sharing (from the info page, or by long-pressing a row in the list) writes the plugin back out as a `.inu.js` file and opens the share sheet, so it arrives as a file the install flow reads
...
- plugins can write formatted text: `inu.utils.md`/`html`/`thtml` are tagged templates that answer the text plus its entities (an interpolated value is never markup, and interpolated formatted text brings its own entities along), with `unparse`/`escape` for the other direction; bulletins, dialogs and settings rows (button, select, separator) take formatted text wherever they took a plain string
- "start in safe mode" launcher shortcut to boot without running any plugin; the next start is also safe when the app died while a plugin was starting, or crashed twice in a row within 30s of its plugins starting; active safe mode shows a banner on the plugins page with a restart button
- plugins can compile a JS subset into JVM routines that execute on the calling thread without JS callbacks, with local variables, loops, conditions, arithmetic, try/catch/finally, grant checks and unload cancellation; input checks preserve guards and exception handlers, and `inu verify` checks the recorded source against bytecode after bundling. Xposed before/after phases accept Java Runnables or Consumers, with routines for editing arguments/results/throwables
- Android plugins can define JVM subclasses and interface implementations with instance/static fields, constructors, and JS or interpreted-routine method bodies (`inu.jvm.defineClass`); the host names the class unless one is asked for, since a loaded dex stays loaded and a name of the plugin's own can never be defined again; `inu.jvm.callSuper` runs a superclass implementation past an override, from JS and from routines
